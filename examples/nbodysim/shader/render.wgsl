...
struct VertexOutput {
    ...
    @location(1) relative_pos: vec2<f32>,
};
...
fn vs_main(@builtin(vertex_index) vertex_index: u32) -> VertexOutput {
    ...
    output.position = vec4<f32>((pos.xy * 2.0) - vec2(1.0) + OFFSETS[vertex_index % 6] * 0.01, 0.0, 1.0);
...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    return vec4<f32>(vec3(1.0, 1.0, in.velocity) * alpha, alpha);
}
